    ...
    while len(lines) > 0:
        ...
        if cur_section == section.DATA.value:
            ...
            if temp == 'la':
                ...
                text_seg.write('lui\t%s\t%s\n' % (token_line[1], target[:-4]))
                text_section_size += BYTES_PER_WORD
                address += BYTES_PER_WORD
                if not target.endswith('0000'):
                    text_seg.write('ori\t%s\t%s\t0x%s\n' % (token_line[1], token_line[1], target[-4:]))
                    text_section_size += BYTES_PER_WORD
                    address += BYTES_PER_WORD
            elif temp == 'move':
                text_seg.write('addi\t%s\t%s,\t0\n' % (token_line[1], token_line[2]))
                text_section_size += BYTES_PER_WORD
                address += BYTES_PER_WORD
            elif temp == 'blt':
                text_seg.write('slt\t$1,\t%s\t%s\n' % (token_line[1], token_line[2]))
                text_seg.write('bne\t$1,\t0,\t%s\n' % (token_line[3]))
                text_section_size += BYTES_PER_WORD * 2
                address += BYTES_PER_WORD * 2
            elif temp == 'push':
                STACK_POINTER = '$29'
                text_seg.write('addi\t%s,\t%s,\t-4\n' % (STACK_POINTER, STACK_POINTER))
                text_seg.write('sw\t%s\t0(%s)\n' % (token_line[1], STACK_POINTER))
                text_section_size += BYTES_PER_WORD * 2
                address += BYTES_PER_WORD * 2
            elif temp == 'pop':
                STACK_POINTER = '$29'
                text_seg.write('lw\t%s\t0(%s)\n' % (token_line[1], STACK_POINTER))
                text_seg.write('addi\t%s,\t%s,\t4\n' % (STACK_POINTER, STACK_POINTER))
                text_section_size += BYTES_PER_WORD * 2
                address += BYTES_PER_WORD * 2
            else:
...
def record_text_section(fout):
    ...
    for line in lines:
        ...
        inst_obj = [x for x in inst_list if x.name == token_line[0]][0]
        inst_type = inst_obj.type
        op = inst_obj.op
        funct = inst_obj.funct
        name = inst_obj.name
        ...
        if inst_type == 'R':
            ...
            for i in range(1, len(token_line)):
                token_line[i] = int(''.join([x for x in token_line[i] if x.isdigit()]))

            if name in ['sll', 'srl']:
                rd, rt, shamt = token_line[1:]
            elif name in ['jr']:
                rs = token_line[1]
            else:
                rd, rs, rt = token_line[1:]

            fout.write(op)
            fout.write(num_to_bits(rs, 5))
            fout.write(num_to_bits(rt, 5))
            fout.write(num_to_bits(rd, 5))
            fout.write(num_to_bits(shamt, 5))
            fout.write(funct)
        ...
        if inst_type == 'I':
            ...
            blank
            '''
            if token_line[-1].find('(') != -1:
                tmp = token_line.pop(-1).split('(')
                token_line.extend(tmp)
            for i in range(1, len(token_line)):
                token_line[i] = ''.join([x for x in token_line[i] if x != '$' and x != ',' and x != ')'])

            if name in ['beq', 'bne']:
                rs, rt, imm = token_line[1:]
            elif name in ['lui']:
                rt, imm = token_line[1:]
            elif name in ['lw', 'sw']:
                rt, imm, rs = token_line[1:]
            else:
                rt, rs, imm = token_line[1:]

            fout.write(op)
            fout.write(num_to_bits(int(rs), 5))
            fout.write(num_to_bits(int(rt), 5))
            if convert_label(imm):
                imm = convert_label(imm) >> 2
            elif imm.find('x') != -1:
                imm = int(imm, 16)
            else:
                imm = int(imm)
            ...
            if name in ['beq', 'bne']:
                imm = (imm - (cur_addr >> 2)) - 1
            fout.write(num_to_bits(imm, 16))
            print(rt, rs, imm)
        ...
        if inst_type == 'J':
            ...
            addr = token_line[1]
            fout.write(op)
            fout.write(num_to_bits(convert_label(addr) >> 2, 26))
